  ...
  return (
    <>
      <motion.div
        ...
        style={{
          background:
            'linear-gradient(to bottom, rgba(21, 29, 44, 0.25), rgba(11, 16, 24, 0.25))',
        }}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: false }}
      ...
      >
        <div className="flex items-center h-full justify-between">
          <NavLink
            to={"/Home"}
            className={"uppercase tracking-widest md:text-[31px] text-[21px] font-semibold"}
          >
            ByteWise
          </NavLink>
          <ul className="lg:flex hidden items-center justify-center lg:gap-[32px] gap-[16px] z-40">
            {links.map(({ to, label }) => (
              <FlipLink key={to} to={to}>
                {label}
              </FlipLink>
            ))}
          </ul>
          <div className="flex item-center justify-center gap-[14px]">
            <Languages />
            <div
              className="relative lg:hidden flex p-[1px] rounded-[16px] bg-gradient-to-b from-[rgba(244,244,245,0.4)] to-[rgba(142,142,143,0)] bg-clip-padding border border-transparent"
      ...
      <AnimatePresence>
        {isNav && (
          <motion.div
            className="fixed bg-GradientBG lg:hidden flex flex-col items-center justify-center gap-[32px] inset-0 z-40"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
          ...
          >
            ...
            {links.map(({ to, label }, index) => (
              ...
              >
                <FlipLink to={to} setIsNav={setIsNav}>
                  {label}
                </FlipLink>
              </motion.div>
            ))}
